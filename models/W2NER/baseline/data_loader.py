...
def load_data_bert(config, data=None):
    with open(os.getcwd()+'/models/W2NER/baseline/data/{}/test.json'.format(config.dataset), 'r', encoding='utf-8') as f:
        test_data = json.load(f)

    test_data[0]['sentence'] = list(data)

    tokenizer = AutoTokenizer.from_pretrained(r"./models/W2NER/baseline/our_bert", cache_dir=r"./models/W2NER/baseline/cache")

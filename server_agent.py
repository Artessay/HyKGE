...
parser.add_argument('--config', type=str, default=r'./config/agent_DOC_example.json')
...
def mapping_func(data_dict, histroy_query_buffer, config: Config):
    ...
    if function_call_type == 'NER':
        # HO Module
        print("NER Module start")
        starttime = datetime.datetime.now()
        ner_res, sim_threshold = NER_module.process(origin_data)
        endtime = datetime.datetime.now()
    ...
    if function_call_type == 'KG':
        ...
        path_prompt = KG_module.fetchKGPath(ner_res, sim_threshold, origin_data)

        return path_prompt, histroy_query_buffer

    if function_call_type == 'Baike':
        # Baike Module
        starttime = datetime.datetime.now()
        print("NER Module start")
        ner_res, sim_threshold = NER_module.process(origin_data)
        ...
        entity_description = KG_module.processDescription_ragent(ner_res, origin_data)
        endtime = datetime.datetime.now()
        print("KG Module(description) finished in :", (endtime - starttime).seconds, " seconds")
        starttime = endtime

        return str(entity_description), histroy_query_buffer


    if function_call_type == 'DOC':
        # TODO ADD DOC => 注意和后面的文件名称对应关系。
        starttime = datetime.datetime.now()
        print("DOC Module start")
        sentence_list = DOC_module.processKnowledge(origin_data)
        endtime = datetime.datetime.now()
        print("DOC Module finished in :", (endtime - starttime).seconds, " seconds")
        starttime = endtime
        print(sentence_list)
        return sentence_list, histroy_query_buffer


    # NOTE STAGE: AFTER RETRIEVAL
    # Filter Module
    # print("Filter Module start")
    # # todo reranker 把下面的注释恢复
    ...
    if function_call_type == 'KO':
        print("KO Module start")
        starttime = datetime.datetime.now()
        KO_knowledge = KO_module.process([], [], [], origin_data)
        endtime = datetime.datetime.now()
        print("KO Module finished in :", (endtime - starttime).seconds," seconds")
        starttime = endtime
        return KO_knowledge, histroy_query_buffer
